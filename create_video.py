...
def merge_audio_video_and_cut():
    ...
    try:
        ...
        # Note: 'pos' tuples needs to be integers or floats relative to the parent clip dimensions.
        # Ensure IMAGE_OVERLAY_PATH exists in the same directory as the script.
        # Calculation for vertical offset might need adjustment based on actual image sizes.
        logo_clip = (ImageClip(IMAGE_OVERLAY_PATH)
                     .set_duration(5.0)
                     .set_start(0.5)
                     .crossfadein(1.0).crossfadeout(1.0)
                     .set_pos(('center', (final_video_clip.h / 2) - (ImageClip(IMAGE_OVERLAY_PATH).h / 2) - 600))) # Vertical offset -600 (check this offset)
        
        # Ensure TEXT_OVERLAY_PATH exists in the same directory as the script.
